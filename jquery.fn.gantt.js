            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                       ...
                       /*Custome code for adding class for milestone and task*/ 
                       var custClassName = ''
                        var custClassDesc = ''
                        if(entry.name == 'Milestone'){
                            custClassName = 'mileName'
                            custClassDesc = 'mileDesc'
                        }else if(entry.name == 'Task'){
                            custClassName = 'taskName'
                            custClassDesc = 'taskDesc'
                        }else{
                            custClassName = ''
                            custClassDesc = ''
                        }/* END */

                        entries.push('<div class="row name row' + i + (entry.desc ? '' : ' fn-wide') +  ' '+custClassName+' " id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '">');
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '">' + (entry.name || '') + '</span>');
                        entries.push('</div>');

                        if (entry.desc) {
                            entries.push('<div class="row desc row' + i + ' '+custClassDesc+' " id="RowdId_' + i + '" data-id="' + entry.id + '">');
